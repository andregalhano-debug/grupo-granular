...
import { ArrowLeft, CalendarDays, List, Settings, Check, X, Lock, Users, BarChart2, FlaskConical } from 'lucide-react'
...
import { MentorLeadsView } from '../components/admin/MentorLeadsView'
import { MentorReportsView } from '../components/admin/MentorReportsView'
import { seedMentorLeads } from '../services/mentorService'

type AdminTab = 'bookings' | 'slots' | 'calendar' | 'mentores' | 'relatorios'
          ...
          {bookings.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map((b) => (
            <div key={b.id} className="rounded-xl bg-white border border-[#0E0E0F]/5 p-4">
              <div className="flex items-start justify-between gap-3 mb-3">
                ...
                <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full ${statusColors[b.status]}`}>{b.status}</span>
              </div>
              ...
              {b.status === 'pendente' && (
                <div className="flex items-center gap-2">
                  <button type="button" onClick={() => handleStatus(b.id, 'confirmada')} className="flex items-center gap-1 text-xs font-medium text-green-600 bg-green-50 hover:bg-green-100 px-3 py-1.5 rounded-lg transition-colors cursor-pointer"><Check size={12} /> Confirmar</button>
                  <button type="button" onClick={() => handleStatus(b.id, 'cancelada')} className="flex items-center gap-1 text-xs font-medium text-red-600 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg transition-colors cursor-pointer"><X size={12} /> Cancelar</button>
                </div>
              )}
            </div>
          ))}
...
function SlotsConfigView() {
  ...
  const toggleDay = (day: number) => {
    ...
    setConfig(updated); saveDemoConfig(updated)
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <p className="text-xs font-medium text-[#9C958A] uppercase tracking-wider mb-3" style={{ fontFamily: "'JetBrains Mono', monospace" }}>Dias da semana</p>
        <div className="flex flex-wrap gap-2">
          {dayNames.map((name, i) => (
            <button key={i} type="button" onClick={() => toggleDay(i)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all cursor-pointer ${config.days.includes(i) ? 'bg-[#A31631] text-white' : 'border border-[#9C958A]/20 text-[#9C958A] hover:border-[#A31631]/30'}`}>{name}</button>
          ))}
        </div>
      </div>
      <div>
        <p className="text-xs font-medium text-[#9C958A] uppercase tracking-wider mb-3" style={{ fontFamily: "'JetBrains Mono', monospace" }}>Horários</p>
        <div className="flex flex-wrap gap-2">
          {allTimes.map((time) => (
            <button key={time} type="button" onClick={() => toggleTime(time)} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all cursor-pointer ${config.times.includes(time) ? 'bg-[#A31631] text-white' : 'border border-[#9C958A]/20 text-[#9C958A] hover:border-[#A31631]/30'}`}>{time}</button>
          ))}
        </div>
      </div>
    ...
    <div className="space-y-6">
      ...
      {connected ? (
        <div className="rounded-xl bg-green-50 border border-green-200 p-4 sm:p-6 text-center">
          ...
          <button type="button" onClick={() => { localStorage.removeItem('granular-gcal'); setConnected(false) }} className="text-xs text-red-600 font-medium cursor-pointer hover:underline">Desconectar</button>
        </div>
      ) : (
        <div className="rounded-xl bg-white border border-[#0E0E0F]/10 p-4 sm:p-6 text-center">
          ...
          <p className="text-xs text-[#9C958A] mb-4">Sincronize os agendamentos de demonstração com seu calendário.</p>
          <button type="button" onClick={() => { localStorage.setItem('granular-gcal', '1'); setConnected(true) }} className="inline-flex items-center gap-2 bg-[#A31631] hover:bg-[#7A1025] text-white font-medium px-6 py-3 rounded-xl text-sm transition-colors cursor-pointer">
...
export function AdminPage() {
  ...
  const [tab, setTab] = useState<AdminTab>('mentores')
  ...
  const tabs: { id: AdminTab; icon: typeof List; label: string }[] = [
    ...
    { id: 'bookings',   icon: List,          label: 'Demos' },
  ...
  return (
    <div className="min-h-screen bg-[#F7F7F7]">
      <header className="bg-white border-b border-[#0E0E0F]/10 px-4 sm:px-6 py-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link to="/" className="text-[#9C958A] hover:text-[#0E0E0F] transition-colors"><ArrowLeft size={20} /></Link>
            <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={handleSeed}
              title="Popula o CRM com mentores de exemplo para visualizar o sistema"
              className={`flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg border transition-colors cursor-pointer ${seeded ? 'bg-green-50 text-green-600 border-green-200' : 'border-[#9C958A]/20 text-[#9C958A] hover:border-[#0E0E0F]/30 hover:text-[#0E0E0F]'}`}
            >
              <FlaskConical size={13} />
              {seeded ? 'Adicionado!' : 'Dados de teste'}
            </button>
            <button type="button" onClick={() => { sessionStorage.removeItem('granular-admin'); setAuthed(false) }} className="text-xs text-[#9C958A] hover:text-[#0E0E0F] transition-colors cursor-pointer">
              Sair
            </button>
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
        <div className="flex gap-1.5 mb-6 sm:mb-8 overflow-x-auto pb-1 -mx-1 px-1">
          {tabs.map((t) => (
            <button key={t.id} type="button" onClick={() => setTab(t.id)}
              className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium whitespace-nowrap transition-colors cursor-pointer ${tab === t.id ? 'bg-[#A31631]/10 text-[#A31631]' : 'text-[#9C958A] hover:bg-white hover:text-[#0E0E0F]'}`}>
              <t.icon size={16} />
              {t.label}
            </button>
          ))}
        ...
        {tab === 'mentores'   && <MentorLeadsView />}
        {tab === 'relatorios' && <MentorReportsView />}
